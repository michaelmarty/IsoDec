...
class PhaseModel:
    ...
    def __init__(self, working_dir=None, phaseres=4):
        """
        ...
        if working_dir is None:
            if platform.system() == "Linux":
                ...
                working_dir = os.path.join(os.path.dirname(os.path.abspath(filename)), "modelparams")
            else:
    ...
    def model_size(self):
        ...
        model_tparams = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
        print(f"Model has {model_tparams} trainable parameters")

        # Dimensions of the model
        for name, param in self.model.named_parameters():
            print(f"Layer: {name} | Size: {param.size()} | Requires Grad: {param.requires_grad}")
        return model_params, model_tparams
    ...
    def load_model(self, savepath=None):
        ...
        if savepath is not None:
            self.savepath = os.path.join(self.working_dir, savepath)
    ...
    def save_model(self, savepath=None):
        ...
        :param savepath: Path to save the model. If None, uses the current savepath.
        :return: None
        """
        if self.model is None:
            self.setup_model()
